...
const CommentsModal = (prop) => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            const res = await axios.get(`https://backend-wjln.onrender.com/api/comments/${prop.post._id}`)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post("https://backend-wjln.onrender.com/api/comments", newComment);
...
export const EditPostModal = (prop) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put("https://backend-wjln.onrender.com/api/posts/" + prop.post._id, updatedPost, config);
            setDesc("")
            setFile([]);
            setImagePreview([]);
            cancel();
...
export const EditProfileModal = (prop) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const userUpdated = await axios.put("https://backend-wjln.onrender.com/api/users/" + user._id, updatedUser, config);
...
export const ResetPasswordModal = (prop) => {
    ...
    const templateId = 'template_g5s35ze';
    const emailjsUserId = 'tWfJrhth-0PKbt-1X';
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await axios.get("https://backend-wjln.onrender.com/api/auth/reset/" + email);

            emailjs.send('service_dsv07nq', templateId, {
                email: email,
                name: res.data.name,
...
export const ResetPasswordModalTopbar = (prop) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            try {
                const userUpdated = await axios.put("https://backend-wjln.onrender.com/api/users/" + user._id, updatedUser);
                localStorage.setItem("user", JSON.stringify(userUpdated.data))
                cancel();
...
export const HelpModal = (prop) => {
    ...
    const templateId = 'template_ygdi6ev';
    const emailjsUserId = 'tWfJrhth-0PKbt-1X';
