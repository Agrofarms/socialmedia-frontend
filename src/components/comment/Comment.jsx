...
export default function Comment({ comment }) {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            const res = await axios.get(`https://backend-wjln.onrender.com/api/users?userId=${comment.userId}`);
    ...
    const deleteComment = async () => {
        ...
        try {
            await axios.delete("https://backend-wjln.onrender.com/api/comments/" + comment._id, { data: obj });
            setDeleted(true);
            commentDeleted();
        } catch (err) {
            console.log(err);
